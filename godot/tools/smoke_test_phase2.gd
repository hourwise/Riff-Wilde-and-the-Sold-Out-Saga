...
func _test_player_visual(player: Node3D) -> void:
	...
	# Facing has been wrong three times, each caught only by playing the game.
	# Measured here from the skeleton so it fails in CI instead.
...
##
## On any humanoid rig the toe bone sits in front of the ankle, so foot-to-toes
## points where the character faces — independent of naming, art pack or rest
## pose. Measured in the CharacterVisual's space so the configured yaw correction
## is included. Godot's forward is -Z.
func _measure_facing(character: Node3D) -> Vector3:
	...
	var forward: Vector3 = (to_visual * skeleton.get_bone_global_pose(toe_index)).origin \
		- (to_visual * skeleton.get_bone_global_pose(foot_index)).origin
	forward.y = 0.0
